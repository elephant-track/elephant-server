...
ELEPHANT_IMAGE_NAME?=elephant-server:0.7.0-dev
...
ELEPHANT_RABBITMQ_USE_SSL?=false
ELEPHANT_RABBITMQ_SSL_VERIFY?=verify_none
ELEPHANT_RABBITMQ_SSL_FAIL_IF_NO_PEER_CERT?=false
...
launch: warmup
	...
	-e RABBITMQ_USE_SSL=$(ELEPHANT_RABBITMQ_USE_SSL) \
	-e RABBITMQ_SSL_VERIFY=$(ELEPHANT_RABBITMQ_SSL_VERIFY) \
	-e RABBITMQ_SSL_FAIL_IF_NO_PEER_CERT=$(ELEPHANT_RABBITMQ_SSL_FAIL_IF_NO_PEER_CERT) \
...
apptainer-launch:
	...
	if [ $(ELEPHANT_GPU) = all ]; then \
		...
		--env RABBITMQ_USE_SSL=$(ELEPHANT_RABBITMQ_USE_SSL) \
		--env RABBITMQ_SSL_VERIFY=$(ELEPHANT_RABBITMQ_SSL_VERIFY) \
		--env RABBITMQ_SSL_FAIL_IF_NO_PEER_CERT=$(ELEPHANT_RABBITMQ_SSL_FAIL_IF_NO_PEER_CERT) \
